...
pcoa.meio.bray.plot
pcoa.meio.bray.plot + annotate("text", x =6, y = 9, label = "PERMANOVA: p-value < 0.05\n PERMDISP: p-value > 0.05")
#ggsave(filename = "nonenzymaticmetabolites_pcoa_allmetabolites", width = 20, height = 15, units = "cm", device = "pdf", dpi = "print")


########################################################################################################################################################################################################
##2. Using log-transformed and scaled data (no centering) (Answer - PERMANOVA p < 0.05 (significance is weak 0.042; and no pairwise difference) and permdisp p < 0.05 ##########################################################################################################################################################################################################################################################################################################################################################################################################################
## Statistical analysis on dist matrix ----
## Combine the data first ###
metabolites_all_logtransformed_scaled_2_metadata <- cbind(metabolites_all_logtransformed_scaled_2, metadata)
metabolites_all_logtransformed_scaled_2_metadata$date <- factor(metabolites_all_logtransformed_scaled_2_metadata$date)
metabolites_all_logtransformed_scaled_2_metadata$day.time <- factor(metabolites_all_logtransformed_scaled_2_metadata$day.time )

#1. PERMANOVA#######################################################################################################################################################################################################################################################################################
#Note: 1. always check if the column is right! it could be not. 2. The result always changes because of the random permutation and so the significance return will always be different. 
adonis.nonenzymaticmetabolites_logtransformed_scaled <- adonis(metabolites_all_logtransformed_scaled_2_metadata[,1:15] ~ metabolites_all_logtransformed_scaled_2_metadata$day.time, method = "euclidean")
adonis.nonenzymaticmetabolites_logtransformed_scaled$aov.tab

#                                                       Df  SumsOfSqs    MeanSqs F.Model      R2 Pr(>F)  
#metabolites_all_logtransformed_scaled_2_metadata$day.time  3 0.00038448 1.2816e-04  2.1402 0.31441  0.042 *
#Residuals                                                 14 0.00083837 5.9884e-05         0.68559         
#Total                                                     17 0.00122286                    1.00000     

#Post-hoc test: I plan to use fdr as correction method (used in QIIME2 as well)
pairwise.adonis(metabolites_all_logtransformed_scaled_2_metadata[,1:15], metabolites_all_logtransformed_scaled_2_metadata$day.time, 
                                p.adjust.m ='fdr', sim.method = 'euclidean') # no pairwise difference 

#2. PERMDISP######################################################################################################################################################################################################################################################################################
metabolites_all_logtransformed_scaled_2_metadata_euclidean_dist <- vegdist(metabolites_all_logtransformed_scaled_2_metadata[,1:15], method = "euclidean") 

bdisp <- betadisper(metabolites_all_logtransformed_scaled_2_metadata_euclidean_dist, metabolites_all_logtransformed_scaled_2_metadata$day.time, type=c("centroid"))
bdisp   
aov.bdisp <-anova(bdisp)
aov.bdisp       #aov and permutest (next code) produced the same result - with minor difference in the p-value -see Pat Schloss video on the interpretation CC208
permutest(bdisp, pairwise = TRUE) #Significant : How to interpret more: See this http://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html
...
#Response: Distances
#              Df     Sum Sq    Mean Sq      F N.Perm Pr(>F)  
#Groups     3 5.5946e-05 1.8649e-05 4.1857    999  0.035 *
#Residuals 14 6.2374e-05 4.4553e-06    
...
# calculate principal coordinates analysis (Euclidean)
euclidean.pcoa.eigen <- cmdscale(metabolites_all_logtransformed_scaled_2_metadata_euclidean_dist, k = 2, eig = T)
...
#Calculate the proportion of variance in the data which is explained by the first two PCoA axes
(euclidean.pcoa.eigen$eig[1]/(sum(euclidean.pcoa.eigen$eig)))*100 #47.09443%

(euclidean.pcoa.eigen$eig[2]/(sum(euclidean.pcoa.eigen$eig)))*100 #28.05409%
...
pcoa.meio.bray.plot + annotate("text", x = 0.02, y = 0.010, label = "PERMANOVA: p-value < 0.05\n PERMDISP: p-value < 0.05")
#ggsave(filename = "nonenzymaticmetabolites_pcoa_allmetabolites", width = 20, height = 15, units = "cm", device = "pdf", dpi = "print")

########################################################################################################################################################################################################
##3. Using log-transformed,centered scaled data (answer - (PERMANOVA: p < 0.05 (super strong significance); PERMDISP: p > 0.05 ##########################################################################################################################################################################################################################################################################################################################################################################################################################
## Statistical analysis on dist matrix ----
## Combine the data first ###
metabolites_all_logtransformed_centerscaled_metadata <- cbind(metabolites_all_logtransformed_centerscaled, metadata)
metabolites_all_logtransformed_centerscaled_metadata$date <- factor(metabolites_all_logtransformed_centerscaled_metadata$date)
metabolites_all_logtransformed_centerscaled_metadata$day.time <- factor(metabolites_all_logtransformed_centerscaled_metadata$day.time )

#1. PERMANOVA#######################################################################################################################################################################################################################################################################################
#Note: 1. always check if the column is right! it could be not. 2. The result always changes because of the random permutation and so the significance return will always be different. 
adonis.nonenzymaticmetabolites_logtransformed_centerscaled <- adonis(metabolites_all_logtransformed_centerscaled_metadata[,1:15] ~ metabolites_all_logtransformed_centerscaled_metadata$day.time, method = "euclidean")
adonis.nonenzymaticmetabolites_logtransformed_centerscaled$aov.tab

#                                                      Df SumsOfSqs MeanSqs F.Model      R2 Pr(>F)   
#metabolites_all_logtransformed_centerscaled_metadata$day.time  3    89.586  29.862  2.5274 0.35132  0.007 **
#Residuals                                                     14   165.414  11.815         0.64868          
#Total                                                         17   255.000                 1.00000   

#Post-hoc test: I plan to use fdr as correction method (used in QIIME2 as well)
pairwise.adonis(metabolites_all_logtransformed_centerscaled_metadata[,1:15], metabolites_all_logtransformed_centerscaled_metadata$day.time, 
                p.adjust.m ='fdr', sim.method = 'euclidean') # no pairwise difference 

#2. PERMDISP######################################################################################################################################################################################################################################################################################
metabolites_all_logtransformed_centerscaled_metadata_euclidean_dist <- vegdist(metabolites_all_logtransformed_centerscaled_metadata[,1:15], method = "euclidean") 

bdisp <- betadisper(metabolites_all_logtransformed_centerscaled_metadata_euclidean_dist, metabolites_all_logtransformed_centerscaled_metadata$day.time, type=c("centroid"))
bdisp   
aov.bdisp <-anova(bdisp)
aov.bdisp       #aov and permutest (next code) produced the same result - with minor difference in the p-value -see Pat Schloss video on the interpretation CC208
permutest(bdisp, pairwise = TRUE) #Significant : How to interpret more: See this http://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html
...
#Response: Distances
#               Df Sum Sq Mean Sq      F N.Perm Pr(>F)
#Groups     3 3.5407  1.1803 2.2998    999  0.115
#Residuals 14 7.1848  0.5132
...
# calculate principal coordinates analysis (Euclidean)
euclidean.pcoa.eigen <- cmdscale(metabolites_all_logtransformed_centerscaled_metadata_euclidean_dist, k = 2, eig = T)
...
#Calculate the proportion of variance in the data which is explained by the first two PCoA axes
(euclidean.pcoa.eigen$eig[1]/(sum(euclidean.pcoa.eigen$eig)))*100 #39.33%

(euclidean.pcoa.eigen$eig[2]/(sum(euclidean.pcoa.eigen$eig)))*100 #30.25%
...
pcoa.meio.bray.plot
pcoa.meio.bray.plot + annotate("text", x = 5.5, y = 8, label = "PERMANOVA: p-value < 0.05\n PERMDISP: p-value > 0.05")
#ggsave(filename = "nonenzymaticmetabolites_pcoa_allmetabolites", width = 20, height = 15, units = "cm", device = "pdf", dpi = "print")
